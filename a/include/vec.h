...
#define VECTOR_ITEM(VEC, IDX) ({ \
    ...
    typeof(VEC) VECTOR_ITEM_vec = VEC; \
    typeof((typeof(*VECTOR_ITEM_vec->data))VECTOR_INVALID_INDEX) VECTOR_ITEM_result = (typeof(*VECTOR_ITEM_vec->data))VECTOR_INVALID_INDEX; \
...
#define VECTOR_REMOVE_BY_VALUE(VEC, VALUE) do { \
    typeof(VEC) VECTOR_REMOVE_BY_VALUE_vec = VEC; \
    typeof(VALUE) VECTOR_REMOVE_BY_VALUE_v = VALUE; \
...
#define VECTOR_FOR_EACH(VEC, BINDING, ...) do { \
    typeof(VEC) VECTOR_FOR_EACH_vec = VEC; \
    for (size_t VECTOR_FOR_EACH_i = 0; VECTOR_FOR_EACH_i < VECTOR_FOR_EACH_vec->length; VECTOR_FOR_EACH_i++) { \
        typeof(&VECTOR_FOR_EACH_vec->data[VECTOR_FOR_EACH_i]) BINDING = &VECTOR_FOR_EACH_vec->data[VECTOR_FOR_EACH_i]; \
        __VA_ARGS__ \
    } \
} while (0)
